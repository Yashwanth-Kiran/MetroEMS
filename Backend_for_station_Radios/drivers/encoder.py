...
class EncoderDriver(DeviceDriver):
    vendor = "generic-encoder"
    device_type = "encoder"
    ...
    def identify(self, ip: str, c: str = "public") -> Dict[str, Any]:
        ...
        match = False
        if isinstance(sysdescr, str) and any(k in sysdescr.lower() for k in ["encoder", "video encoder", "stream encoder"]):
            match = True
