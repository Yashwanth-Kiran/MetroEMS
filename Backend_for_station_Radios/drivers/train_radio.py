...
class TrainRadioDriver(DeviceDriver):
    vendor = "generic-train-radio"
    device_type = "train_radio"
    enterprise_oid_prefix = None  # Unknown vendor; rely on heuristics
    ...
    def identify(self, ip: str, c: str = "public") -> Dict[str, Any]:
        ...
        match = False
        if isinstance(sysdescr, str) and any(k in sysdescr.lower() for k in ["train", "rail", "radio"]):
            match = True
